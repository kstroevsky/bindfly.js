...
export interface CreateMovingPointSimulationInput {
	...
	readonly parameters: MovingPointParameters
}
...
const createBuffer = (capacity: number): MovingPointBuffer => ({
	count: 0,
	capacity,
...
class MovingPointSimulation implements CheckpointableMovingPointSimulation {
	...
	private readonly parameters: MovingPointParameters
	private random: RandomSource
	private nextId = 0

	constructor(environment: SimulationEnvironment, parameters: MovingPointParameters) {
		...
		this.state = { particles: createBuffer(capacityFor(parameters.particleCount)) }
		this.reset()
	}
	...
	step(frame: SimulationStep): void {
		if (!Number.isFinite(frame.dtSeconds) || frame.dtSeconds < 0) {
			throw new RangeError('Moving point dtSeconds must be finite and non-negative.')
		}
		const particles = this.state.particles
		const minX = Math.min(this.parameters.margin, this.viewport.cssWidth / 2)
		const minY = Math.min(this.parameters.margin, this.viewport.cssHeight / 2)
	...
	applyInput(input: MovingPointInput): void {
		const particles = this.state.particles
		switch (input.type) {
			case 'add-point':
				if (particles.count >= MAXIMUM_MOVING_POINT_COUNT) break
