...
test('hot map parameter updates preserve the orbit and change the next iteration', () => {
	...
	assert.equal(changed.orbits[0]?.x, 1)
	assert.equal(changed.configurationEpoch, 1)
	assert.deepEqual(changed.orbits[0]?.trailEpochs.slice(-2), [0, 1])
	session.step({ index: 2, dtSeconds: 1 / 12, elapsedSeconds: 2 / 12 })
	const after = session.snapshot()
	assert.equal(after.orbits[0]?.x, 3)
	assert.equal(after.orbits[0]?.trailEpochs.at(-1), 1)
