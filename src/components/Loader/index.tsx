...
import { insertToArray } from '../../shared/utils';
...
const Loader: FC<ILoaderProps> = ({ size, style, center }) => {
    ...
    const tileIndexToMove = (state: ILoaderState) => state.stateNumber === 7 ? 4 : state.stateNumber % state.positions.length

    const positionForTile = (positions: ILoaderState['positions'], radioCommand: string | null) =>
        positions.findIndex((position: string | null) => position === radioCommand)

    const setNextState = () => setState(prev => {
        const indexToMove = tileIndexToMove(prev);
        const currentPositions = [...prev.positions]

        currentPositions[positionForTile(currentPositions, null)] = prev.positions[indexToMove]
    ...
    return (
        ...
        >
            <div className={'sw-loader__holder'}>
                {initialState.positions.map((radioCommand: string | null) => (
                    radioCommand === null || <LoaderTiles
                        {...{
