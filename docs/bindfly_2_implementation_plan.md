...
**Status (2026-09-25): in progress.** The transport-independent collaboration foundation is implemented and recorded in `docs/v2/stage-16a-report.md`: an in-memory authoritative room assigns ordered sequences, validates and authorizes untrusted inputs, enforces idempotency and input-size limits, and produces versioned SHA-256 snapshots. Client replicas buffer sequence gaps, reject stale scheduled events, detect divergence and resynchronize only from checksum-verified compatible snapshots. The first real deterministic checkpoint covers moving-point particle state plus RNG and ID-allocation state, and an integration test proves two replicas remain identical after out-of-order/duplicate delivery, forced divergence, resynchronization and subsequent random-dependent point creation. WebSocket transport, reconnect handshake, authoritative distributed step/tick scheduling, persistent room storage and the dedicated privacy/security/operations review remain Stage 16 work.
