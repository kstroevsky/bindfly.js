...
**Status (2026-09-25): in progress through Stage 16B.** The transport-independent collaboration foundation is recorded in `docs/v2/stage-16a-report.md`, and authoritative scheduling/reconnect semantics are recorded in `docs/v2/stage-16b-report.md`. The authority now separates accepted log head from the event prefix already applied to simulation state, schedules inputs at future fixed-step boundaries, emits `AuthoritativeTick` watermarks, enforces input-size plus per-participant step limits, and creates snapshots that cover only applied state. Replicas cannot advance beyond a tick while an applied authoritative event is missing and reject contradictory event/tick scheduling metadata. Reconnect deterministically chooses ordered suffix replay or a checksum-verified snapshot plus post-snapshot pending events; a current checksum mismatch forces snapshot recovery. Moving-point integration tests prove replay recovery, snapshot fallback and subsequent random-dependent convergence with RNG/ID identity preserved. WebSocket transport, persistent room storage and the dedicated privacy/security/operations review remain Stage 16 work.
