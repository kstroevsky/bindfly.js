...
**Status (2026-09-26): in progress through Stage 16C.** The transport-independent collaboration foundation is recorded in `docs/v2/stage-16a-report.md`, authoritative scheduling/reconnect semantics in `docs/v2/stage-16b-report.md`, and the first real WebSocket transport in `docs/v2/stage-16c-report.md`. The authority separates accepted log head from the event prefix already applied to simulation state, schedules inputs at future fixed-step boundaries, emits `AuthoritativeTick` watermarks, enforces input-size plus per-participant step limits, and creates snapshots that cover only applied state. Replicas cannot advance beyond a tick while an applied authoritative event is missing and reject contradictory event/tick scheduling metadata. Reconnect deterministically chooses ordered suffix replay or a checksum-verified snapshot plus post-snapshot pending events; a current checksum mismatch forces snapshot recovery. The WebSocket server authenticates before upgrade, binds participant identity to the connection, requires resume before submit, serializes room mutations and broadcasts authoritative events/ticks. Real WebSocket integration tests prove ordered delivery, replay recovery, snapshot fallback and subsequent random-dependent convergence with RNG/ID identity preserved. Persistent room storage, production canonical configuration bytes and the dedicated privacy/security/operations review remain Stage 16 work.
