...
**Status (2026-09-26): complete through Stage 16E.** Reports `docs/v2/stage-16a-report.md` through `docs/v2/stage-16e-report.md` record the transport-independent authority/replica foundation, fixed-step scheduling and reconnect semantics, real WebSocket delivery, durable authoritative-room recovery and the dedicated privacy/security/operations review. The authority separates accepted log head from applied state, schedules canonical inputs at future fixed-step boundaries, emits `AuthoritativeTick` watermarks and recovers divergent/stale replicas through replay or checksum-verified snapshots. Snapshot identity is SHA-256 over canonical binary snapshot bytes, complete moving-point checkpoints preserve RNG/ID identity, and the Flying Lines collaboration configuration now has versioned canonical bytes for seed, parameters, shared simulation dimensions and fixed-step duration. The WebSocket server authenticates before upgrade, binds participant identity, requires resume before submit, enforces wire/input/rate/connection limits, persists accepted state before acknowledgement/broadcast and fails closed if durable storage becomes unavailable. Real WebSocket tests cover ordering, duplicate/out-of-order recovery, replay, snapshot fallback, process restart with a pending event and post-recovery deterministic convergence. Production deployment conditions and the single-process/file-store scaling boundary are explicit in the Stage 16E review.
