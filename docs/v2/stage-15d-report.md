...
Environment: Chromium 153.0.8010.12, Apple M1 Max, 1440×900 browser viewport, DPR 1. The measured code is commit `5ddca316c5483a43f36c2baeac9c099801d732e5`; tracked files were clean during the run. The same Stage 15C parity protocol and frozen workloads were used.
...
Relative to the Stage 15C WebGL2 run, median upload cost changed from 0.50→0.40 ms for Flying Lines, 0.40→0.20 ms for Vector Field and 0.10→0.00 ms for Discrete Map. Vector Field's median WebGL CPU frame changed from 0.60→0.40 ms, removing the representation-conversion penalty that made it slower than Canvas2D in Stage 15C.

Scalar Field was deliberately unchanged. Its cached raster/contour path already measured zero steady-state WebGL upload cost, so Stage 15 provides no evidence for a float-texture path, shader colormap, GLSL formula evaluator or GPU simulation tier.

`EXT_disjoint_timer_query_webgl2` remained unavailable in the benchmark Chromium environment. The report therefore makes no claim about absolute GPU execution time. Browser timer resolution also quantizes these sub-millisecond samples at roughly 0.1 ms, so the numbers are evidence about bottleneck location and this workload/machine, not universal backend performance guarantees.
